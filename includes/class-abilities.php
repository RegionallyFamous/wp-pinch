...
class Abilities {
	...
	public static function generate_tldr_for_post( int $post_id ) {
		if ( ! Feature_Flags::is_enabled( 'molt' ) ) {
			return new \WP_Error( 'molt_disabled', __( 'Molt is not enabled.', 'wp-pinch' ) );
		}
		...
		return '' !== $summary ? $summary : new \WP_Error( 'empty_summary', __( 'Molt returned an empty summary.', 'wp-pinch' ) );
	}

	/**
	 * Execute generate-tldr ability: generate and store TL;DR in post meta.
	 *
	 * @param array $input Ability input.
	 * @return array
	 */
	...
	/**
	 * Execute suggest-links ability: return posts that are good link candidates.
	 *
	 * @param array $input Ability input.
	 * @return array
	 */
	public static function execute_suggest_links( array $input ): array {
		...
		if ( $post_id ) {
			$related = self::execute_related_posts(
				array(
					'post_id' => $post_id,
					'limit'   => $limit,
				)
	...
	/**
	 * Execute quote-bank ability: extract notable sentences from a post.
	 *
	 * @param array $input Ability input.
	 * @return array
	 */
	...
	/**
	 * Execute what-do-i-know ability: query → search + synthesize → answer + source IDs.
	 *
	 * @param array $input Ability input.
	 * @return array
	 */
	public static function execute_what_do_i_know( array $input ): array {
		...
		$synthesize_result = self::execute_synthesize(
			array(
				'query'     => $query,
				'per_page'  => $per_page,
				'post_type' => 'post',
	...
	/**
	 * Execute project-assembly ability: weave multiple posts into one draft.
	 *
	 * @param array $input Ability input.
	 * @return array
	 */
	...
	/**
	 * Execute knowledge-graph ability: nodes (posts, terms) and edges (links, shared terms).
	 *
	 * @param array $input Ability input.
	 * @return array
	 */
	public static function execute_knowledge_graph( array $input ): array {
		...
		$nodes      = array();
		$edges      = array();
		$link_edges = array(); // from_id => [ to_id, ... ] from content links
		...
		foreach ( $posts as $post ) {
			...
			// Extract links to other posts/pages.
		...
		// Shared-term edges: two posts share at least one term.
		$term_nodes = array();
		$post_terms = array(); // Maps post ID to list of term IDs.
		foreach ( $posts as $post ) {
	...
	/**
	 * Execute find-similar ability: related posts by search + taxonomy (MVP; no embeddings yet).
	 *
	 * @param array $input Ability input.
	 * @return array
	 */
	...
	/**
	 * Execute spaced-resurfacing ability: list posts not updated in N days.
	 *
	 * @param array $input Ability input.
	 * @return array
	 */
